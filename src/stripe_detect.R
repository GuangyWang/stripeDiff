...
compareStripe <- function(list1, list2){
  ...
  colnames(stripe1) <- c('upPeak.loc', 'downPeak.loc', 'leftEdge', 'rightEdge', 'upPeak.sample1', 
                         ...
                         'downPeak.sample2', 'logFoldChange.sample2', 'stripe.pValue.sample2')
  ...
  colnames(stripe2) <- c('upPeak.loc', 'downPeak.loc', 'leftEdge', 'rightEdge', 'upPeak.sample2', 
                         ...
                         'downPeak.sample1', 'logFoldChange.sample1', 'stripe.pValue.sample1')
...
stripeLocation <- function(stripe, contactMap, upLimit, j, p, outputName5 = 'NA', outputName6 = 'NA', outputName7='NA', direct){
  ...
  if(direct == 'left'){
    ...
    stripe.left = min(nrow(contactMap) - stripe[j,1], nrow(contactMap) - stripe[j,2])
    stripe.right = max(nrow(contactMap) - stripe[j,1], nrow(contactMap) - stripe[j,2])
  }
