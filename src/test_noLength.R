...
getStrap <- function(contactMap, upLimit, outputPath='NA', outputName1='NA', 
  ...
  matchPeaks <- function(upSide, downSide, p, map){
    ...
    if( !all(is.na(upLocalPeak)) & !all(is.na(downLocalPeak)) & is.matrix(upPeak)){
      ...
      if(nrow(strap)>0){
        down.peak = strap[,2]
        down.peak = down.peak[!duplicated(down.peak)]
        strap2 = strap
        strap2[,3] = abs(strap2[,2]-strap2[,1])
        strap3 = data.frame()
        for(d in down.peak){                            # match up peak and down peak
          strap2.temp = strap2[strap2[,2] == d,]
          index2 = which.min(strap2.temp[,3])
          strap3 = rbind(strap3, strap2.temp[index2,1:2])
        }
...
compareStrap <- function(list1, list2){
  ...
  colnames(strap2) <- c('upPeak.loc', 'downPeak.loc', 'leftEdge', 'rightEdge', 'upPeak.sample2', 
                        ...
                        'downPeak.sample1', 'logFoldChange.sample1', 'strap.pValue.sample1')
